...
from schemas.Livepeer import LivepeerStream
...
async def create_new_stream(stream: LivepeerStream):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.post(LIVEPEER_BASE_URL+'/stream', json=json_body, headers=HEADERS)
        if resp.status_code == 201:
            ...
            return {
                'stream_id': stream_id, 'playbackId':playbackId, 'streamKey':streamKey
            }
...
async def get_stream_status(stream_id: str):
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.get(f'{LIVEPEER_BASE_URL}/stream/{stream_id}', headers=HEADERS)
        if resp:
            resp = resp.json()
            return resp
        return "Some error occured. Please try again later."